...
struct TokenLoginTests {
    @Test func buildsAuthorizeURLWithScopesAndCallback() throws {
        ...
        let url = try #require(TokenLogin.authorizeURL(instance: URL(string: "http://10.0.1.9:3001")!, callback: callback))
        let components = try #require(URLComponents(url: url, resolvingAgainstBaseURL: false))
        #expect(components.host == "10.0.1.9")
        #expect(components.port == 3001)
