...
struct StoryboardTests {
    ...
    @Test func parsesCues() {
        let track = StoryboardTrack.parse(webVTT: vtt, relativeTo: URL(string: "http://10.0.1.9:3001"))
        #expect(track.cues.count == 3)
        #expect(track.cues[1].x == 160)
        #expect(track.cues[2].start == 3610)
        #expect(track.cues[2].imageURL.absoluteString == "http://10.0.1.9:3001/sb/ID/M1.jpg")
    }
